...
def update_ck(graph, src, dests, ck_vals, k, ignored_node):
    ...
    while not frontier.empty() and path_length <= k:
        ...
        for neighbor in graph[cur]:
            if neighbor not in explored and neighbor is not ignored_node:
...
def get_ck_node(graph, node, k):
    ...
    for src in neighbors:
        ...
        dests.remove(src)
        update_ck(graph, src, dests, ck_vals, k, node)
    n = len(neighbors)
    n = max(((n - 1) * n) / 2.0, 1)
